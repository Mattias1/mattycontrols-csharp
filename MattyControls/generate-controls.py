...
filename_objects_from   = 'MattyControls.template.cs'
filename_objects_to     = 'MattyControls.cs'
filename_interface_from = 'IMattyControl.template.cs'
filename_interface_to   = 'IMattyControl.cs'
...
class Control:
    ...
    def writeToFile(self, fto, controlcopy):
        ...
        w()
        w('    public class {} : {}, IMattyControl'.format(self.typename, self.basetype))
        w('    {')
        w(self.constructor)
        w(controlcopy)
        w('    }')
...
# Handle the file IO for the object
with open(filename_objects_to, 'w') as fto:
    writeNotice(fto)

    with open(filename_objects_from) as ffrom:
        sourceLines = []
...
# Handle the file IO for the interface
with open(filename_interface_to, 'w') as fto:
    writeNotice(fto)

    with open(filename_interface_from) as ffrom:
        sourceLines = []
        loopCompiler = LoopPreCompiler()
        for line in ffrom:
            loopCompiler.compile(sourceLines, line)
        fto.write(''.join(sourceLines)) # The sourcelines already include a newline char
